...
public class UserMapper {
    ...
    public static UserDocument mapToDocument(User user){
        return new UserDocument(user.getId(), user.getUsername(), user.getPassword(), user.getEmail(), user.getRoles(), user.isStatus(),user.getPdfs());
    }

    public static User mapToModel(UserDocument userDocument){
        return new User(userDocument.getId(), userDocument.getUsername(), userDocument.getPassword(), userDocument.getEmail(), userDocument.getRoles(), userDocument.isStatus(),userDocument.getPdfs());
    }
}
